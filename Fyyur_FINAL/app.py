...
agenre_table = db.Table(
    'agenre_table',
    db.Column('genre_id', db.Integer, db.ForeignKey('Genre.id'), primary_key=True),
    db.Column('artist_id', db.Integer, db.ForeignKey('Artist.id'), primary_key=True)
)

vgenre_table = db.Table(
  'vgenre_table',
  db.Column('genre_id', db.Integer, db.ForeignKey('Genre.id'), primary_key=True),
...
class Venue(db.Model):
    ...
    genres = db.relationship('Genre', secondary=vgenre_table, backref=db.backreff('venues'))
...
class Artist(db.Model):
    ...
    genres = db.relationship('Genre', secondary=agenre_table, backref=db.backreff('artist'))
    website = db.Column(db.String(120))
    seeking_talent = db.Column(db.Boolean, default=False)
    seeking_description = db.Column(db.String(120))
...
class Show(db.Model):
  __tablename__= 'Show'
    ...
    start_time = db.Column(db.DateTime, default=datetime.astimezone, nullable=False)

    artist_id = db.Column(db.Integer, nullable=False, db.ForeignKey('Artist_id'))
    venue_id = db.Column(db.Integer, nullable=False, db.ForeignKey('Venue.id'))

    def __repr__(self):
      return f'<Venue {self.id} {self.start_time} artist_id={artist_id} venue_id={venue_id}>'
...
def venues():
  ...
  cities_states.sort(key=itemgetter(0,1))

  for loc in cities_states:
    venues_list = []
    for venue in venues:
      if(venue.city == loc[1]) and (venue.state == loc[0]):
...
def create_venue_submission():
  ...
  city = city.name.data.strip()
  state = form.state.data
  address = form.address.data.strip()
  phone = form.phone.data
  genres =form.genres.data
  seeking_talent = True if form.seeking_talent.data == 'Yes'
                        else False
...
def delete_venue(venue_id):
  venue = Venue.query.delete(venue_id)
  if not venue:
    return redirect(url_for('index'))
  else:
    ...
    expect:
      ...
      db.session.close()
    if error_on_delete:
      flash(f'An error occurred deleting {venue_name}.')
...
def edit_artist(artist_id):
  ...
  if not artist
    ...
    from = ArtistForm(obj=artist)
  genre = [genre.name for genre in artist.genre]
  artist = {
    "id": artist_id,
    "name": artist.name,
    "genres": genres,
...
def edit_artist_submission(artist_id):
    ...
    genres = form.genres.data 
    seeking_venue = True if form.seeking_venue.data == 'Yes' 
                        else False
    seeking_description = form.seeking_description.data.strip()
    image_link = form.image_link.data.strip()
    website = form.website.data.strip()
...
def delete_artist(artist_id):
  ...
  return redirect(url_for('index'))
    ...
    db.session.close()
  if error_on_delete:
    flash(f'An error has occurred. Deleteing {artist_name}')
...
def create_show_submission():
  ...
  artists_id = form.artist_id.data.strip()
  venue_id = form.venue_id.data.strip()

  try:
    new_show = Show(artist_id, venue_id)
    db,session.add(New_Show)

  db.session.commit()
  if:
    error_in_insert = True
    error_message = f'{artist_id} does not exsist within our database, add one instead'
    print(error_message)
    db.session.rollback()
  else:
    db.session.close()
  if error_in_insert:
    error_message_1 = f'This show could not be listed!'
    print(error_message_1)
  else:
    error_message_2 = f'This show was sucessfully listed!'
